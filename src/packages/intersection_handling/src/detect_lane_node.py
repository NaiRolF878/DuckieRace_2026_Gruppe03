#!/usr/bin/env python3
"""
DetectLaneNode – Spurerkennung via Bird's-Eye-View.

Publiziert:
  /{vehicle}/detect/lane  (Float64) – Spurversatz [-1, +1]
                                      > 0 → zu weit links   → rechts lenken
                                      < 0 → zu weit rechts  → links lenken

Hinweis: Rote-Linie-Erkennung wurde in detect_apriltag_node ausgelagert.
"""
...
class DetectLaneNode:
    def __init__(self, node_name):
        ...
        # Zustandsvariablen für run_debug
        self.img              = None
    ...
    def get_x_for_driving(self, mask, distance, no_lane_value, left_line):
        grad = cv2.Sobel(mask, cv2.CV_16S, 1, 0,
        ...
        for row in range(distance - 50, distance + 50):
            cols = np.where(th1[row] == 255)[0]
            if cols.size == 0:
                continue
            a.append(cols[-1] if left_line else cols[0])

        return np.median(a) if len(a) > 10 else no_lane_value
    ...
    def cbFindLane(self, image_msg):
        ...
        try:
            ...
            rospy.logdebug(f"[detect_lane] error={msg_error.data:.3f}")
            ...
            # Live-Fenster
            image = img.copy()
            image = cv2.circle(image, (int(lane_center), int(len(img) / 2)), 3, (255, 0, 0))
            image = cv2.line(image, (white_alternative,  0), (white_alternative,  self._crop_im_size), (255, 255, 255))
            image = cv2.line(image, (yellow_alternative, 0), (yellow_alternative, self._crop_im_size), (255, 255, 0))
            image = cv2.line(image, (0, int(len(img) * 0.75) + 100), (len(img[0]), int(len(img) * 0.75) + 100), (255, 255, 255))
            image = cv2.line(image, (0, int(len(img) * 0.75) - 100), (len(img[0]), int(len(img) * 0.75) - 100), (255, 255, 255))
            ...
            rospy.logerr(f"[detect_lane] Fehler: {e}")
        finally:
            self.is_running = False
    ...
    def run_debug(self):
        ...
        while not rospy.is_shutdown():
            if self.pub_debug_lane.get_num_connections() > 0 and self.img is not None:
                debug_img = self.img.copy()
                debug_img = cv2.circle(debug_img, (int(self.lane_center), int(len(debug_img) / 2)), 3, (255, 0, 0))
                debug_img = cv2.line(debug_img, (self.white_alternative,  0), (self.white_alternative,  1000), (255, 255, 255))
                debug_img = cv2.line(debug_img, (self.yellow_alternative, 0), (self.yellow_alternative, 1000), (255, 255, 0))
                debug_img = cv2.line(debug_img, (0, int(len(debug_img) * 0.75) + 100), (len(debug_img[0]), int(len(debug_img) * 0.75) + 100), (255, 255, 255))
                debug_img = cv2.line(debug_img, (0, int(len(debug_img) * 0.75) - 100), (len(debug_img[0]), int(len(debug_img) * 0.75) - 100), (255, 255, 255))
                ...
                msg.format = "jpeg"
                msg.data   = np.array(cv2.imencode('.jpg', debug_img)[1]).tobytes()
                self.pub_debug_lane.publish(msg)

            if self.pub_debug_white.get_num_connections() > 0 and self.debug_img_white is not None:
                msg = CompressedImage()
                msg.header.stamp = rospy.Time.now()
                msg.format = "jpeg"
                msg.data   = np.array(cv2.imencode('.jpg', self.debug_img_white)[1]).tobytes()
                self.pub_debug_white.publish(msg)

            if self.pub_debug_yellow.get_num_connections() > 0 and self.debug_img_yellow is not None:
                msg = CompressedImage()
                msg.header.stamp = rospy.Time.now()
                msg.format = "jpeg"
                msg.data   = np.array(cv2.imencode('.jpg', self.debug_img_yellow)[1]).tobytes()
                self.pub_debug_yellow.publish(msg)
