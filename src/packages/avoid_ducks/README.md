...
### Kurzes Positions-Gedächtnis für Enten, um Bot-Drehung korrigiert (`cb_ducks`/`_project_remembered_bboxes`)
...
Reines Einfrieren der letzten Bildposition wäre aber selbst innerhalb der Gedächtniszeit falsch, sobald sich der Bot währenddessen weiterdreht (genau der Fall beim Ausweichen) – die Zone, in der die Ente laut Bild steht, würde nicht mehr stimmen. `_project_remembered_bboxes()` korrigiert die gemerkte Position deshalb bei jedem Aufruf per Odometrie: eine Drehung um `Δθ` (aus `self.theta`) verschiebt ein stillstehendes Objekt im Bild horizontal um ungefähr `fx · Δθ` Pixel (Lochkamera-Modell, Kleinwinkel-Näherung, `fx` aus den Kamera-Intrinsics `self.K`). Dreht der Bot z.B. nach links, wandert die gemerkte Position im Bild entsprechend nach rechts. Mit Odometrie-Simulation getestet und bestätigt (0.2 rad Drehung bei `fx=300` → Verschiebung um 60 Pixel in die erwartete Richtung).

**Grenzen der Näherung:** korrigiert nur die reine Blickrichtungsänderung (Drehung) – berücksichtigt weder eigene Vor-/Rückwärtsbewegung des Bots noch eine mögliche Eigenbewegung der Ente selbst. Für die kurze Gedächtniszeit (Bruchteile einer Sekunde) ist das ein vertretbarer Kompromiss.
